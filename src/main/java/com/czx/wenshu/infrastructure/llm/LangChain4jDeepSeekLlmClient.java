...
public class LangChain4jDeepSeekLlmClient implements LlmClient {
    ...
    private static final Logger log = LoggerFactory.getLogger(LangChain4jDeepSeekLlmClient.class);
    ...
    public String chat(String systemPrompt, String userPrompt) {
        ...
        try {
            ...
            log.info("[DeepSeekClient] LLM 调用完成 输入字符={} 输出字符={} 耗时={}ms", inputTokens, result != null ? result.length() : 0, elapsed);
            return result;
        } catch (Exception e) {
            log.warn("[DeepSeekClient] LLM 调用失败 输入字符={} 耗时={}ms", inputTokens, System.currentTimeMillis() - start);
