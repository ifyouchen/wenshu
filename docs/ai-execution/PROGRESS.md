...
整体状态：P0~P9 **全部完成**，规范对照分析发现的 17 处偏差**全部修复**（180 个测试通过，前端构建成功）。P8 22/22，P9 9/9。P10 DEFERRED（V2.0 生态能力）。
...
| P8 前端工作台 | DONE | 22/22 | P8 全部完成 |
| P9 商业化与数据安全 | DONE | 9/9 | P9 全部完成 |
...
- P8-17：异常状态 UI 与内容安全说明（`api/safety.ts`；`ErrorStateAlert.vue` 4 级（A/B/C/D）；`AiFloatButton.vue` 集成错误状态展示，`inferErrorLevel` 函数按 HTTP 状态/错误关键词推断级别）。
- P9-03：支付订单与回调验签（Flyway V11 payment_orders；PaymentOrder 领域；PaymentService 占位验签可降级；SubscriptionController 新增 checkout/topup/cancel；WebhookController；8 个集成测试通过）。
- P9-07：团队版成员与共享配额（Flyway V12 teams+team_members；Team/TeamMember 领域；TeamService invite/accept/removeRole 完整流程；TeamController；173 个全量测试通过）。
- P8-19：订阅升级引导 UI（`useUpgradeModal.ts` 单例；`UpgradeModal.vue` 三场景弹窗；ErrorStateAlert C 级直接触发；api/subscription 新增 checkout/topup/cancel）。
- P9-08：团队版作品协作（Flyway V14 project_collaborators；ProjectCollaborator 领域；TeamCollaborationService；CollaboratorController；TeamController 新增 usage 端点；180 个测试通过）。
- P9-09：体验额度与字数包（Flyway V13 word_packs；WordPack 领域；WordPackService；注册自动发放 5万字体验额度；支付 topup 后自动发包；QuotaInfo 扩展 wordPackRemainingChars）。

## 当前待办

**P8 全部完成（22/22），P9 全部完成（9/9）。**  
P10 DEFERRED（V2.0 企业生态能力，包含文风市场/多人实时协作增强/企业 API 开放平台）。  
V1 全主链路后端（P0~P9）+ 前端工作台（P8）均已实现，项目阶段性收尾。

## 实现日志

### 2026-06-16 ★ 全链路规范对照修复（17 处偏差全部解决）

**P0 修复（阻塞级）**
- **P0-1 AI 接受内容流程**：`WritingDailyStats.addAiAcceptedDelta()` 新方法；`WritingStatsService.recordAiAcceptedDelta()` 新方法；`ProjectApplicationService.acceptAiContent()` 新方法（创建 `auto_before_ai` 快照 + 更新 ai_accepted_chars）；`POST /chapters/{id}/accept-ai` 新端点；前端 `AiFloatButton.vue` 添加 `hasPendingAiContent` 状态 + "接受"/"放弃"按钮；`acceptAiContent()` API 函数。
- **P0-2 一致性审查 DeepSeek 分层**：`ConsistencyTaskRunner` 注入 `utilityLlmClient`（DeepSeek 主扫描）+ `creativeLlmClient`（Claude 二次确认，限 5 次/轮）。
- **P0-3 一致性审查配额按套餐分级**：`ConsistencyService` 注入 `SubscriptionService`，通过 `getUserPlanLimits()` 读取用户实际套餐限额；`QuotaService` 新增 `checkAndIncrementAdaptationWithLimit(userId, limit)` 动态限额方法。
- **P0-4 POST /auth/logout-all**：`AuthApplicationService.logoutAll(userId)` 方法；`AuthController.POST /auth/logout-all` 端点；`WebMvcConfig` 新增拦截路径。

**P1 修复（功能规格）**
- **P1-1 写作时段热力图（小时×星期）**：`WritingDailyStats.peakHour` 字段 + `updatePeakHour()` 方法；`WritingDailyStatsMapper` 更新 SELECT/INSERT/UPDATE 含 `peak_hour`；新增 `findTimeHeatmap(userId)` 聚合查询；`WritingStatsService` 在 record 方法中自动调用 `updatePeakHour()`；`WritingStatsQueryService.getTimeHeatmap()` 方法；`GET /stats/writing/time-heatmap` 端点；前端 `StatsView.vue` 新增 7×24 CSS Grid 热力图；Flyway V15 迁移（`ALTER TABLE writing_daily_stats ADD COLUMN IF NOT EXISTS peak_hour INT DEFAULT -1`）。
- **P1-2 快照类型中文映射**：`SnapshotDrawer.vue` 新增 `snapshotTypeLabel()` 函数，将 `manual/auto_before_ai/polish_accepted/auto_before_replace/auto_before_restore` 映射为中文 NTag。
- **P1-3 趋势图三维度切换**：`DailyStats` 接口新增 `manualChars?` / `aiAcceptedChars?` 字段；`StatsView.vue` 新增 `trendDimension` ref + `getTrendValue()` + `NRadioGroup` 切换组件。
- **P1-4 连续日更提示横条**：`EditorView.vue` 加载时调用 `getWritingOverview()`，当 streak > 0 且今日未达标时展示可关闭的琥珀色提示横条。
- **P1-5 专有名词词典容量按套餐限制**：`WorldElementRepository/Mapper` 新增 `countByProjectId()`；`WorldElementApplicationService.createWorldElement()` 检查容量上限（free=50/pro=300/enterprise=无限）。

**P2 修复（轻微 / 商业完整性）**
- **P2-2 AI 操作前消耗预估**：`AiFloatButton.vue` 续写操作下方增加"预计消耗约 800-1500 字配额"小字提示。
- **P2-3 趋势图下载为图片**：`StatsView.vue` 新增 `downloadTrendChart()` 函数（SVG 序列化→Blob→下载）+ 下载按钮。
- **P2-4 导入后异步元数据提取**：`ImportApplicationService.applyImport()` 后调用 `CharacterAnchorService` 对前 3 章进行锚点提取。
- **P2-5 多标签页心跳检测**：`useEditorHeartbeat.ts` composable（localStorage 心跳 3s 间隔 + 6s 过期，按 chapterId 隔离）；`EditorView.vue` 接入，另一标签页同时编辑时显示橙色非阻断提示条。
- **P2-6 团队角色三级系统**：`TeamMember.ALLOWED_ROLES = Set.of("admin","editor","writer")`；`changeRole()` 枚举校验；`TeamController.RoleRequest` 正则更新为 `admin|editor|writer`。
- **P2-1 邀请奖励 UI 占位**：`SettingsView.vue` 订阅用量 Tab 增加邀请好友赠额度说明区域（"即将上线"状态）。
- 全量回归 **180 个后端测试**通过；前端 `npm run build` ✅ TypeScript 无错误。

### 2026-06-16 P8-19 / P9-08 / P9-09 ★ P8+P9 全部完成

- **P8-19 订阅升级引导 UI**：`useUpgradeModal.ts` 模块级单例（visible/scenario/openUpgrade/closeUpgrade）；`UpgradeModal.vue`（Teleport to body；NModal；三场景：quota-chars/quota-adaptations/pro-feature，对应配额用尽/改编用尽/专业版入口；展示专业版 5 大特性列表；套餐卡片（从 /subscriptions/plans 加载，过滤免费版）；"立即升级"调用 `createCheckout` POST /subscriptions/checkout，payUrl 无效时展示友好提示；`@after-enter` 懒加载套餐数据）；`MainLayout.vue` 挂载 `<UpgradeModal />`；`ErrorStateAlert.vue` C 级"升级套餐"按钮改为调用 `openUpgrade('quota-chars')`；`api/subscription.ts` 新增 `OrderInfo` 类型及 createCheckout/createTopup/cancelSubscription 函数。
- **P9-08 团队版作品协作与统一账单**：Flyway V14 `project_collaborators` 表；`ProjectCollaborator` 领域（add/rehydrate/changeRole，角色 editor/viewer）；`ProjectCollaboratorRepository`+MyBatis 持久化；`TeamCollaborationService`（addCollaborator 权限校验 + removeCollaborator + listCollaborators + getTeamUsage 按成员汇总 QuotaInfo）；`CollaboratorController`（POST/GET/DELETE `/projects/{id}/collaborators/**`，需鉴权）；`TeamController` 新增 GET `/teams/{id}/usage`；WebMvcConfig 新增 `/projects/*/collaborators/**` 路径。
- **P9-09 体验额度与字数包**：Flyway V13 `word_packs` 表（pack_type trial/topup，expires_at 永久为 null）；`WordPack` 领域（createTrial 5万字/createTopup/consume/remainingChars/isEffective）；`WordPackRepository`+MyBatis 持久化；`WordPackService`（issueTrial 幂等防重发/issueTopup/getTotalRemainingChars/consumeFromPacks FIFO/listActivePacks）；`AuthApplicationService` 注册时自动 `issueTrial()`；`PaymentService` 支付 topup 后自动 `issueTopup()`；`QuotaInfo` 新增 `wordPackRemainingChars` 字段（向后兼容构造器）；`UserController.getQuota()` 合并月度配额+字数包余量。
- `CollaborationAndWordPackTests` **7 个集成测试**（体验额度自动发放/quota 包含 wordPackRemainingChars/添加协作者/非所有者 403/查询列表/移除后为空/团队 usage 统计）全部通过。全量回归 **180 个测试**通过。
- 前端 `npm run build` ✅ TypeScript 无错误，2987 模块构建成功。

### 2026-06-16 P8-17 / P9-03 / P9-07

- **P8-17 异常状态 UI 与内容安全说明**：新增 `api/safety.ts`（getContentPolicy/submitContentAppeal/listContentAppeals + ContentPolicy/AppealInfo 类型）；新增 `ErrorStateAlert.vue`（A/B/C/D 四级，A: 服务不可用/B: 结果不可用/C: 配额不足含升级按钮/D: 内容被替换含 NTooltip 说明+申诉输入框；从 `/api/safety` 提交申诉，跳转设置页面）；更新 `AiFloatButton.vue`（集成 ErrorStateAlert，`inferErrorLevel()` 函数按 HTTP 状态和错误关键词推断 A/B/C/D 级，SSE error/timeout 事件正确设置错误级别，网络异常→A 级，429/配额→C 级，内容安全→D 级）；`npm run build` 构建通过。
- **P9-03 支付订单与回调验签**：Flyway V11 `payment_orders` 表；`PaymentOrder` 领域（create/markPaid/markCancelled，状态流转 pending→paid→cancelled）；`PaymentOrderRepository`+MyBatis 持久化；`PaymentService`（createCheckout/createTopup/processWebhook 占位验签/cancelSubscription；订单号格式 WS{yyyyMMddHHmmss}{random8}；payUrl 无凭据时返回 PAYMENT_NOT_CONFIGURED 占位符）；`SubscriptionController` 扩展（POST /subscriptions/checkout|topup|cancel + CheckoutRequest/TopupRequest 请求 DTO）；`WebhookController`（POST /webhook/payment，无需鉴权，透传 channel/orderNo/channelNo 参数 + raw body 验签）；WebMvcConfig 新增 checkout/topup/cancel 路径。
- **P9-07 团队版成员与共享配额**：Flyway V12 `teams`+`team_members` 表（唯一约束 team_id+user_id，邀请码 UNIQUE）；`Team` 领域（create/updateName，共享配额默认 1000万字/200次改编）；`TeamMember` 领域（invite/createOwner/accept/remove/changeRole，状态 pending→active/removed）；`TeamRepository`+`TeamMemberRepository`+完整 MyBatis 持久化；`TeamService`（createTeam 自动创建 admin owner、invite 生成 INV+16位随机码、acceptInvite 状态校验、listMembers 鉴权、removeMember/changeRole admin 权限检查）；`TeamController`（POST /teams | GET /teams | POST /teams/{id}/invites | POST /teams/invites/{code}/accept | GET/DELETE/PUT /teams/{id}/members/**）；WebMvcConfig 新增 `/teams/**` 路径；测试 schema 新增 teams/team_members 表。
- `PaymentAndTeamTests` **8 个集成测试**（支付订单 401/创建订单/字数包/webhook 验签；团队 401/创建/列表/邀请+接受全流程 2 人成员）全部通过。
- 全量回归 **173 个测试**通过。
...
- **P8-20 账户设置 UI**：全量实现 `SettingsView.vue`（替换占位），5 个 Tab：① 👤个人资料（昵称/头像/邮箱只读，调用 PUT /user/profile）② ✏️创作偏好（NInputNumber 每日字数目标 + NSelect 身份类型）③ 🤖AI 内容（NSwitch AI 训练授权，开关实时同步后端）④ 💎订阅用量（调用 GET /subscriptions/current，NProgress 显示字符/改编次数用量，免费版展示升级占位提示）⑤ 🔒账户安全（修改密码+注销账号）；新增 `api/subscription.ts`（getSubscriptionPlans/getCurrentSubscription + PlanInfo/CurrentSubscriptionInfo 类型）；扩展 `api/user.ts`（updateAiConsent/setIdentityType/deleteAccount）。
- **P9-01 配额模型与扣减**：Flyway V9 迁移（subscription_plans + user_subscriptions 表，预置 free/pro/enterprise 三档套餐数据）；领域层 `SubscriptionPlan`/`UserSubscription` 实体 + 两个仓储接口；基础设施层记录类/Mapper/Repository 实现；应用层 `SubscriptionService`（getAvailablePlans/getCurrentSubscription/getUserPlanLimits）；`QuotaService` 新增 `getQuotaInfoWithLimits(userId, charLimit, adaptLimit)` 动态限额重载方法，与硬编码方法共存向后兼容；测试 schema 追加 subscription_plans/user_subscriptions 表 + DROP TABLE 头部声明。
- **P9-02 订阅方案查询**：`SubscriptionController`（GET /subscriptions/plans 无需鉴权；GET /subscriptions/current 需 Bearer Token）；`WebMvcConfig` 新增 `/subscriptions/current` 拦截路径；`PlanInfo`/`CurrentSubscriptionInfo` 响应 DTO；`SubscriptionControllerTests` 6 个集成测试（套餐列表/免费档限额/专业档限额更高；无 Token 401；新用户自动创建免费订阅；配额嵌套结构验证）。全量回归 156 个测试通过。
- `npm run build` ✅ 2987 模块，TypeScript `vue-tsc --noEmit` ✅ 无错误；`mvn test` ✅ 156 个测试全部通过。
...
| 2026-06-16 | `npm run build`（web/）| PASS | P8-17：异常状态 UI（A/B/C/D 四级），TypeScript 无错误，构建成功 |
| 2026-06-16 | `JAVA_HOME=corretto-21.0.11; mvn test` | PASS | P8-19/P9-08/P9-09：升级引导 UI/项目协作/字数包，180 个测试全部通过 |
| 2026-06-16 | `npm run build`（web/）| PASS | P8-19：订阅升级引导弹窗（三场景），TypeScript 无错误，构建成功 |
| 2026-06-16 | `JAVA_HOME=corretto-21.0.11; mvn test` | PASS | 全链路规范对照修复（P0-1~P2-6 共 17 处），180 个测试全部通过 |
| 2026-06-16 | `npm run build`（web/）| PASS | 前端 17 处偏差全部修复（AI 接受流程/热力图/快照i18n/趋势切换/streak提示/多标签心跳等），构建成功 |
| 2026-06-16 | `JAVA_HOME=corretto-21.0.11; mvn test` | PASS | P8-17/P9-03/P9-07：异常状态 UI/支付订单/团队管理，173 个测试全部通过 |
