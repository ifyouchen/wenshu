...
| ID | 状态 | 任务 | 依赖 | 验收 |
| --- | --- | --- | --- | --- |
| P3-01 | DONE | 角色 CRUD | P2 | `/projects/{id}/characters` 可用 |
| P3-02 | DONE | 角色锁定/解锁 | P3-01 | 锁定角色优先进入上下文 |
| P3-03 | DONE | 世界观要素 CRUD | P2 | 可管理地点、势力、道具、规则 |
| P3-04 | DONE | 专有名词词典 CRUD | P2 | 支持别名、类型、定义、锁定 |
| P3-05 | DONE | 角色名与词典同步策略 | P3-01,P3-04 | 角色名更新时词典一致 |
...
| P5-10 | DONE | 文风档案与即时预览 | P5-09 | 样本保存后异步分析标签 |

## P6 一致性审查与锚点

| ID | 状态 | 任务 | 依赖 | 验收 |
| --- | --- | --- | --- | --- |
| P6-01 | DONE | 章节摘要生成 | P2,P5 | 章节完成后可异步生成摘要 |
| P6-02 | DONE | 角色锚点自动提取 | P3,P5 | 章节保存/完成触发档案更新 |
...
| P8-04 | DONE | 登录/注册/邮箱验证/忘记密码 UI | P1 | 对齐 IDD §0 |
| P8-05 | DONE | 作品首页、卡片、空状态、配额 Tooltip | P2,P4 | 对齐 IDD §1 |
| P8-06 | DONE | TipTap 章节编辑器 | P2 | 只加载当前章节 |
| P8-07 | DONE | 大纲、角色库、词典侧栏 | P2,P3 | 左侧图标面板 |
| P8-08 | DONE | AI 浮窗、SSE 插入、AI 内容标识 | P5 | RAF 批量写入 |
| P8-09 | DONE | 搜索替换横条 | P4 | 300ms debounce |
| P8-10 | DONE | 写作统计看板 | P4 | 趋势、热力图、作品进度 |
| P8-11 | DONE | 版本快照和 diff | P2 | 支持只看差异 |
| P8-12 | DONE | 一致性审查报告 | P6 | 可跳转章节和处理条目 |
| P8-13 | DONE | 剧本四栏工作台 | P7 | 场景目录、原文、剧本、AI 建议 |
| P8-14 | DONE | 移动端响应式 | P8 | 核心写作可用，复杂功能限制提示 |
| P8-15 | DONE | 命令面板与键盘导航 | P8-06 | Cmd/Ctrl+K 打开，支持功能搜索、键盘导航、底部快捷提示 |
| P8-16 | DONE | Toast 通知系统 | P8-01 | 右下角堆叠、成功/警告/错误/信息四类、移动端顶部居中 |
| P8-17 | DONE | 异常状态 UI 与内容安全说明 | P8-08,P9-05 | A/B/C/D 级异常展示、内容替换 Tooltip、申诉入口 |
| P8-18 | DONE | 渐进式用户引导 | P8-05,P8-06 | 首次进入、首次选中文字、首次接受 AI、写够 3000 字等触发 |
| P8-19 | DOING | 订阅升级引导 UI | P9 | 专业版功能、配额用尽、改编次数用尽弹窗分场景展示 |
| P8-20 | DONE | 账户设置 UI | P1,P9 | 个人资料、订阅用量、创作偏好、AI 内容设置、数据与隐私 |
| P8-21 | DONE | 快捷键参考面板 | P8-15 | `?` 或命令面板打开，覆盖导航、写作、AI 操作、视图切换 |
| P8-22 | DONE | 前端性能边界与代码分割 | P8-01 | TipTap/剧本工作台/diff 独立分包，移动端不下载重组件 |

## P9 商业化与数据安全

| ID | 状态 | 任务 | 依赖 | 验收 |
| --- | --- | --- | --- | --- |
| P9-01 | DONE | 配额模型与扣减 | P1,P5 | 操作前预估，完成后扣减 |
| P9-02 | DONE | 订阅方案查询 | P1 | `/subscriptions/plans` |
| P9-03 | DONE | 支付订单与回调验签 | P9-02 | 微信/支付宝回调可验签 |
| P9-04 | DONE | 数据导出 ZIP | P2,P3,P7 | 异步导出用户全部数据 |
| P9-05 | DONE | 内容安全过滤和申诉入口 | P5 | 输出替换透明展示 |
| P9-06 | DONE | 版权免责、AI 辅助标识与隐私策略 | P1,P5,P7 | 作品默认私有、导出标注 AI 辅助、隐私和授权可配置 |
| P9-07 | DONE | 团队版成员与共享配额 | P1,P9-01 | 团队管理员邀请成员，成员共享团队配额池 |
| P9-08 | DOING | 团队版作品协作与统一账单 | P9-07,P2 | 项目协作者权限、按人/作品统计用量 |
| P9-09 | DOING | 体验额度与字数包 | P9-01,P9-03 | 免费一次性体验额度、月度配额、不过期字数包可叠加 |
